...
function MarketCoinFull(props) {
  const handleFluctuation = (change) => {
    return Number(change) >= 0 ? (
      <div className="full-coin-fluctuation full-upward-fluctuation">
        {"+" + Number(change).toFixed(2) + "%"}
      </div>
    ) : (
      <div className="full-coin-fluctuation full-downward-fluctuation">
        {Number(change).toFixed(2) + "%"}
      </div>
    );
  };
    ...
    <Link to={`/explore/${props.id}`} key={props.id}>
      <div className="market-coin-full">
        ...
        <div className="full-coin-prices">
          ...
          <div className="full-coin-volume">{handleMoney(props.volume)}</div>
